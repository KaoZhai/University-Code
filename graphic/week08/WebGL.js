...
var FSHADER_SOURCE = `
    ...
    uniform sampler2D u_LVSampler;
    uniform sampler2D u_HESampler;
    varying vec3 v_Normal;
    varying vec3 v_PositionInWorld;
    varying vec2 v_TexCoord;
    void main(){
        ...
        if(distance( texColor, vec3(0.0, 0.0, 1.0) ) < 0.6){
          texColor = texture2D( u_LVSampler, v_TexCoord ).rgb;
        }
        if(distance( texColor, vec3(1.0, 0.0, 0.0) ) < 0.3 && distance( texture2D( u_HESampler, v_TexCoord ).rgb, vec3(1.0, 1.0, 1.0) ) < 0.7){
          texColor = texture2D( u_Sampler, v_TexCoord ).rgb * 0.4 + texture2D( u_HESampler, v_TexCoord ).rgb * 0.6;
        }
...
async function main(){
    ...
    program.u_LVSampler = gl.getUniformLocation(program, "u_LVSampler")
    program.u_HESampler = gl.getUniformLocation(program, "u_HESampler")
...
function draw(){
    ...
    modelMatrix.setRotate(angleY*3, 1, 0, 0);//for mouse rotation
    modelMatrix.rotate(angleX*3, 0, 1, 0);//for mouse rotation
    ...
    gl.uniform1i(program.u_LVSampler, 1);

    gl.activeTexture(gl.TEXTURE2);
    gl.bindTexture(gl.TEXTURE_2D, textures[objCompImgIndex[2]]);
    gl.uniform1i(program.u_HESampler, 2);
