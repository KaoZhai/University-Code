    ...
    public:
        ...
        std::pair<int, int> GetIngredients(std::string ingredient) const {
            ...
            if (table_.FindItem(ingredient, p)) {
                if (pos.first == -1 || Distance(p) < Distance(pos)) {
                    pos = p;
                }
            }
            return pos;
...
int main()
{
    ...
    while (1) {
        ...
        if(nowTodoCustomer.award_ == -1 || customer_list.IsServed(nowTodoCustomer.item_))
        {
            my_chef.ClearTarget();
            nowTodoCustomer = customer_list.GetCustomer(2);
        }

        if(!my_chef.HaveTarget())
        {
            if(my_chef.HaveCarry())
            {
                my_chef.PutOnTable();
                continue;
            }
            ...
            {
                my_chef.SetTarget(nowTodoCustomer.item_);
            }
        }
        ...
        if(my_chef.MyBake())
        {
            if(oven.BakingItem() == CROISSANT || oven.BakingItem() == TART)
            {
                my_chef.Use(OVEN);
                my_chef.EndMine();
                continue;
        ...
        if(my_chef.IsCarrying(DISH))
        {
            if((my_chef.IsCarrying(TART) && (!my_chef.IsTargetContained(TART))) ||
               (my_chef.IsCarrying(CROISSANT) && (!my_chef.IsTargetContained(CROISSANT))) ||
               (my_chef.IsCarrying(CHOPPED_STRAWBERRIES) && (!my_chef.IsTargetContained(CHOPPED_STRAWBERRIES))) ||
               (my_chef.IsCarrying(BLUEBERRIES) && (!my_chef.IsTargetContained(BLUEBERRIES))) ||
               (my_chef.IsCarrying(ICE_CREAM) && (!my_chef.IsTargetContained(ICE_CREAM))))
            {
                bool nextTarget = true;
                for(int i = 2 ; i >= 0 ; --i)
                {
                    my_chef.ClearTarget();
                    nowTodoCustomer = customer_list.GetCustomer(i);
                    my_chef.SetTarget(nowTodoCustomer.item_);
                    if((my_chef.IsCarrying(TART) && (!my_chef.IsTargetContained(TART))) ||
                       (my_chef.IsCarrying(CROISSANT) && (!my_chef.IsTargetContained(CROISSANT))) ||
                       (my_chef.IsCarrying(CHOPPED_STRAWBERRIES) && (!my_chef.IsTargetContained(CHOPPED_STRAWBERRIES))) ||
            ...
            if((my_chef.IsTargetContained(TART) && (!my_chef.IsCarrying(TART))) ||
               (my_chef.IsTargetContained(CROISSANT) && (!my_chef.IsCarrying(CROISSANT))) ||
               (my_chef.IsTargetContained(CHOPPED_STRAWBERRIES) && (!my_chef.IsCarrying(CHOPPED_STRAWBERRIES))))
            {
                if(my_chef.FindTable())
                {
                    continue;
                }
                my_chef.PutOnTable();
            ...
            else if(my_chef.IsTargetContained(BLUEBERRIES) &&  (!my_chef.IsCarrying(BLUEBERRIES)))
            {
                my_chef.MakeGeneral(BLUEBERRIES);
                continue;
            }
            else if(my_chef.IsTargetContained(ICE_CREAM) && (!my_chef.IsCarrying(ICE_CREAM)))
            {
                my_chef.MakeGeneral(ICE_CREAM);
                continue;
            }
        }

        if(my_chef.IsTargetContained(TART) && !(table.FindItem(TART, tempPos) && (!table.HaveItem(tempPos.first, tempPos.second, DISH))))
        {
            if(my_chef.IsCarrying(TART))
            {
                my_chef.PutOnTable();
                continue;
            }
            ...
            {
                if(my_chef.IsCarrying(RAW_TART))
                {
                    if(my_chef.Distance(OVEN) <= 1)
                    {
                        if(oven.BakingItem() == "NONE")
                        {
                            my_chef.Use(OVEN);
                            my_chef.SetMine();
                            continue;
        ...
        if(my_chef.IsTargetContained(CROISSANT) && !(table.FindItem(CROISSANT, tempPos) && (!table.HaveItem(tempPos.first, tempPos.second, DISH))))
        {
            if(my_chef.IsCarrying(CROISSANT))
            {
                my_chef.PutOnTable();
                continue;
            }
            else
            {
                ...
                {
                    my_chef.MakeDoughToOven();
                    continue;
                }
            }
        }
        
        if(my_chef.IsTargetContained(CHOPPED_STRAWBERRIES) && !(table.FindItem(CHOPPED_STRAWBERRIES, tempPos) && (!table.HaveItem(tempPos.first, tempPos.second, DISH))))
        {
            if(my_chef.IsCarrying(CHOPPED_STRAWBERRIES))
            {
                my_chef.PutOnTable();
                continue;
            }
